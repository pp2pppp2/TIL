...
3. 해당 user 레코드 조회
   ...
   ```sql
   -- sql
   SELECT * FROM users_uesr WHERE id=101;
   ```
...
user.delete()
   ...
   ```sql
   -- sql
   DELELTE FROM users_user WHERE id=101;
   ```
...
3. 강원도에 사는 사람의 평균 계좌 잔고
   ...
   ```python
   # orm
   User.objects.filter(county='강원도').aggregate(Avg('balance'))
   ```
...
4. 계좌 잔액 중 가장 높은 값
   ...
   User.objects.order_by('-balance').first().balance
   ```
